odoo.define('phidias_pos_addon.screen', function (require) {
	"use strict";
	...
	screens.PaymentScreenWidget.include({
        ...
        credit_assign: function(e){
            ...
            if($(e.currentTarget).hasClass('account_pay')){
                ...
                self.pos.credit_amount = pos_total_amount;
                if(self.pos.config.pos_journal_id[0]){
                    var cashregisters = null;
                    for ( var j = 0; j < self.pos.payment_methods.length; j++ ) {
                        if ( self.pos.payment_methods[j].id === self.pos.config.pos_journal_id[0] ){
                ...
                if (cashregisters){
                    ...
                    order.selected_paymentline.set_amount( Math.max(paid_amt),0 );                 
		...
		renderElement: function() {
            ...
            if(self.pos.get_order().get_invoice_pay() || self.pos.get_order().get_sale_order_pay()){
                $('.js_gift_card').css('display','none');                
            }
            this.$('#btn_so').click(function(){
            	var order = self.pos.get_order();
            	var lines = [];
                var is_giftcard = false;
                if(order){
                	var currentOrderLines = order.get_orderlines();
                	_.each(currentOrderLines, function(line){
                        if(line.product.invoice_policy == "delivery"){
                    ...
                    if(order.get_paymentlines().length > 0){
                        _.each(order.get_paymentlines(), function(payment_line){
                            if(payment_line.payment_method && payment_line.payment_method.allow_for_gift_card){
                                alert("You can not use Gift Card in Sale Order.")                                
                                is_giftcard = true;
                            }
                        })  
                        if(!is_giftcard){                            
            ...
            this.$('#btn_invoice_pay').click(function(){
                ...
                _.each(order.get_paymentlines(), function(payment_line){
                    if(payment_line.payment_method && payment_line.payment_method.allow_for_gift_card){
                        alert("You can not pay invoice using Gift Card.")                                
                        is_giftcard = true;
                    }
                })    
                if(!is_giftcard){                    
                	if(order.get_due() <= 0 ){
                        ...
                        var params = {
                            model: 'sale.order',
                            method: 'pay_invoice',
                            args: [{ "invoice_id" : invoice_id , "paymentlines" : paymentlines, 'credit_detail': order.get_credit_detail() }],
                        }
	    ...
	    start: function(){
            ...
            this.$('.invoice-list-contents').delegate('#pay_invoice','click',function(event){
            	...
            	if(self.pos.config.paid_amount_product){
                    ...
                    self.gui.show_screen('payment');
                    self.pos.gui.screen_instances.payment.renderElement();
                    $(self.pos.gui.screen_instances.payment.el).find('.button.next, .button.create_so, span#partial_pay').hide();
                }else{
                    self.gui.show_popup('error-traceback',{
                        title: _t("Configuration Required"),
                        body:  _t("Please configure dummy product for paid amount from POS configuration"),
...
//                event.preventDefault();
    ...
    screens.ReceiptScreenWidget.include({
        ...
        render_receipt: function() {
            ...
            }else if(order.get_credit()){
                var receipt_html = QWeb.render('AddedCreditReceipt',this.get_receipt_render_env());
                this.$('.pos-receipt-container').html(receipt_html);
            }else{
            ...
            this._super();
                if(self.pos.get_order().get_client()){
                    var params = {
                        ...
                        args: [self.pos.get_order().get_client().id,self.pos.config.enable_journal_id[0]]
                    }
                    rpc.query(params, {async: false}).then(function(res){                       
                        if(res){
                            self.pos.get_order().get_client()['deposite_info'] = res;                                                    
                        }
                    });                    
                }            
    ...
    var OrderListScreenWidget = screens.ScreenWidget.extend({
        ...
        get_journal_from_order: function(statement_ids){
            ...
            promise.then(function(statements){
                var order_statements = []
                _.each(statements, function(statement){
                    if(statement.amount > 0){
                        order_statements.push({
                            amount: statement.amount,
                            payment_method_ids: statement.payment_method_id[1],
                        })
                    }
                });
                if(order_statements){
        ...
        click_reprint: function(event){
            ...
            if (result && result.lines.length > 0) {
                ...
                if(result.state=="paid"){
                    selectedOrder.set_amount_paid(result.amount_total);
                }else{
                    selectedOrder.set_amount_paid(result.amount_total - result.amount_due);
                }
        ...
        perform_search: function(query, associate_result){
            ...
            if(query){
                if (associate_result){
                    ...
                    var orders = this.pos.db.pos_search_order(query);
                    self.render_list(orders);
                }
            }else{
...
//        Render Order List
        ...
        reloading_orders: function(){
            ...
            var params = {
                ...
                fields: ['create_date', 'state', 'date_order', 'name', 'pos_reference',
                                        'write_date', 'id', 'partner_id', 'lines', 'amount_total','amount_due','company_id','payment_ids'],
            }
    ...
    var AddedCreditReceipt = screens.ScreenWidget.extend({
        template: 'AddedCreditReceipt',
    ...
    gui.define_screen({name:'credit_receipt', widget: AddedCreditReceipt});
